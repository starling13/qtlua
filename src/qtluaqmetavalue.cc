...
namespace QtLua {
  ...
  Value QMetaValue::raw_get_object(State *ls, int type, const void *data)
  {
	...
	Value value(Value::new_table(ls));
	const QVector3D *vector = reinterpret_cast<const QVector3D*>(data);
	value[1] = vector->x();
	value[2] = vector->y();
	value[3] = vector->z();
	return value;
  ...
  void QMetaValue::raw_set_object(int type, void *data, const Value &v)
  {
	...
	*reinterpret_cast<QByteArray*>(data) = v.to_string();
	...
	break;
      ...
      case QMetaType::QVector3D: {
	QVector3D *vector = reinterpret_cast<QVector3D*>(data);
	vector->setX(v.at(1).to_number());
	vector->setY(v.at(2).to_number());
	vector->setZ(v.at(3).to_number());
	break;
      }
